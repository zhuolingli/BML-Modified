...
from abc import ABCMeta, abstractmethod
from mmcls.datasets.pipelines import Compose
from mmfewshot.classification.datasets.pipelines.loading import LoadImageFromBytes
...
img_norm_cfg = dict(
    mean=[120.39586422,  115.59361427, 104.54012653], std=[70.68188272,   68.27635443,  72.54505529], to_rgb=True)

test_pipeline = [
    dict(type='LoadImageFromBytes'),
    # dict(type='Resize', size=(int(84 * 1.15), -1)),
...
class BaseFewShotDataset(Dataset, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.loader = LoadImageFromBytes()
        
        self.label = [item['gt_label'].tolist() for item in self.data_infos]
    ...
    def __getitem__(self, idx: int) -> Dict:
        results = copy.deepcopy(self.data_infos[idx])
        results = self.loader(results)
        img = self.transforms(results['img'])
        return img, results['gt_label']
...
class TieredImageNetDataset(BaseFewShotDataset):
    ...
    def __init__(self,
                ...
                data_prefix='/run/media/cv/d/lzl/fsl/from_50/train/mmfewshot/data/tiered_imagenet',):
        
        self.subset = [subset]
        self.GENERAL_CLASSES = self.get_general_classes()

        super().__init__(data_prefix, classes=None, ann_file=None)
    ...
    def get_classes(
        ...
        if classes is None:
            ...
            for subset_ in self.subset:
                if subset_ == 'train':
                    class_names += [i[0] for i in self.TRAIN_CLASSES] # 具体类别是列表中元组的第一个元素
                elif subset_ == 'val':
    ...
    def get_general_classes(self) -> List[str]:
        ...
        for subset_ in self.subset:
            if subset_ == 'train':
                general_classes += [i[1] for i in self.TRAIN_CLASSES]  # 通用类别是列表中元祖的第二个字符串
            elif subset_ == 'val':
...
if __name__ == '__main__':
    split = 'test'
    data_prefix = '/run/media/cv/d/lzl/fsl/from_50/train/mmfewshot/data/tiered_imagenet'
    dataset = TieredImageNetDataset(subset=split,data_prefix=data_prefix) 
    pass
