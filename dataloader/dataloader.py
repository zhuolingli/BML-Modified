...
from utils.supCon_util import TwoCropTransform
...
def supCon_dataloder(opt, is_val=True):
    # transforms
    train_trans, test_trans, test_trans_plus = transforms_options[opt.transform]
    # # train
    if opt.dataset == "CUB":
        ...
        trainset = ImageNet(args=opt, train_transform=TwoCropTransform(train_trans), test_transform=test_trans, split="train")
    meta_trainloader = DataLoader(dataset=trainset, batch_size=opt.batch_size,
                                  shuffle=True,
                                  num_workers=opt.num_workers,
                                  pin_memory=True)
