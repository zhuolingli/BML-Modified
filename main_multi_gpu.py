...
from dataloader.dataloader import bml_dataloder, global_dataloder, supCon_dataloder
...
from model.SupCon import BaseSupConBuilder
...
def build_model(config):
    ...
    elif config.mode== "supCon":
        return BaseSupConBuilder(config)
    else:
...
def build_dataloader(config, world_size):
    ...
    if config.mode in ["local", "bml"]:
        ...
        return supCon_dataloder(config)
    else:
        raise ValueError('Dont support {}'.format(config.mode))
...
def train_bml_on_epoch(config, model, optimizer, criterion_Intra, criterion_Cross, epoch, meta_trainloader):
    ...
    for idx, (samples, global_targets) in enumerate(meta_trainloader):
        ...
        mutual_loss = criterion_Cross(global_feat, local_feat) + criterion_Cross(local_feat, global_feat) # origin settings 原本是这个loss
...
def train_supCon_on_epoch(config, model, optimizer, epoch, meta_trainloader):
    assert config.mode == 'supCon'
    model.train()
    num_steps = len(meta_trainloader)
    batch_time = AverageMeter()
    loss_meter = AverageMeter()
    criterion = SupConLoss(temperature=config.temp)
    start = time.time()
    end = time.time()
    for idx, (samples, targets) in enumerate(meta_trainloader):
        samples = torch.cat([samples[0], samples[1]], dim=0) # cat the augmentated imgs from 2 branch
        samples = samples.cuda(non_blocking=True)
        targets = targets.cuda(non_blocking=True)
        meta_targets = prepare_label(config, split="train")
        bsz = targets.shape[0]
        
        features = model(samples, split="train") # features has been projected to 128 dim
        f1, f2 = torch.split(features, [bsz, bsz], dim=0)
        
        if config.spatial:
            loss = 0
            
            targets = targets.unsqueeze(1).repeat(1, torch.mul(*f1.shape[-2:])).reshape(-1)
            f1 = f1.flatten(start_dim=2).transpose(2,1).reshape(-1, features.shape[1])
            f2 = f2.flatten(start_dim=2).transpose(2,1).reshape(-1, features.shape[1])
...
def main(config, world_size):
    ...
    dataloader_train, _, dataloader_test = build_dataloader(config, world_size)
    ...
    if config.is_eval:
        load_checkpoint(config, model_without_ddp, logger)
        accuracy = validate(model, dataloader_test, config)
        return
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        if config.mode == "bml":
            train_bml_on_epoch(config, model, optimizer, criterion_Intra, criterion_Cross, epoch, dataloader_train)
        elif config.mode =='supCon':
            train_supCon_on_epoch(config, model, optimizer, epoch, dataloader_train )
        else:
            train_single_on_epoch(config, model, optimizer, criterion_Intra, epoch, dataloader_train)
        # evaluating
        if epoch % config.eval_freq == 0:
            accuracy = validate(model, dataloader_test, config)
            logger.info(f"Accuracy of the network on the {len(dataloader_test) * 4} test images: {accuracy:.2f}%")
